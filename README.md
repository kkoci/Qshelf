...
This completes the registry's initial planned set of five packages, plus a second-round addition. More are likely to follow -- see [CLAUDE.md](CLAUDE.md) for ideas and the full rationale/history behind each package so far.
...
Each package under `packages/` is independent: its own `pyproject.toml`, its own tests, its own README. There's no shared root package -- `packages/qft`, `packages/grover`, `packages/qaoa`, `packages/vqe_h2`, `packages/repetition_code`, `packages/grover_multi` (and future packages) are meant to be installed and consumed individually.

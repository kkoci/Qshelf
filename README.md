...
- **[`packages/grover_multi`](packages/grover_multi/)** -- Grover's search with a genuinely multi-item oracle (marking 2 or 3 items out of the same 8-item register `packages/grover` uses), a second-round addition built on the gotcha #12 workaround `packages/grover` originally used (`grover_multi.py`'s `mark_item` still uses the H-CCX-H workaround; `packages/grover`'s own copy was removed once gotcha #12 was confirmed fixed upstream -- see CLAUDE.md). See its [README](packages/grover_multi/README.md) for the API and the "why a separate package" rationale, and its [tests](packages/grover_multi/tests/test_correctness.py) for how correctness is verified -- against the closed-form multi-item amplitude-amplification formula across several marking patterns and a full iteration-count sweep, run on Selene's statevector emulator.

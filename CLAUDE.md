...
- **VQE for H2** (next up) -- another variational algorithm, but chemistry-flavored; would need a Hamiltonian-encoding convention (e.g. Jordan-Wigner) and expectation-value estimation from measurement statistics (QAOA's `run_qaoa`/`optimize_qaoa` pattern -- compile per parameter set, sample, classically post-process, feed a classical optimizer -- should carry over directly), both new territory for this registry otherwise.

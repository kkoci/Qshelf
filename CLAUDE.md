...
- **Isolate generic-parameter sweeps and independent circuits into separate subprocesses** -- see gotcha #5 above. `packages/qft/tests/_case_runner.py` + `subprocess.run` is the pattern; reuse it for future packages that expose `@guppy.comptime` generics.
